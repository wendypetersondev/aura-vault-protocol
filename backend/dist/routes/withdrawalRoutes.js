...
import { enqueueWithdrawal, getWithdrawalJob, getQueueStats, } from "../services/withdrawalQueue.js";
 ...
 */
withdrawalRouter.post("/", authenticate, userRateLimiter(), async (req, res) => {
    ...
    if (sharesNum > LARGE_WITHDRAWAL_THRESHOLD) {
        const job = enqueueWithdrawal(walletAddress, String(shares), contractId ?? process.env.VAULT_CONTRACT_ID ?? "");
 ...
 */
withdrawalRouter.get("/queue/stats", authenticate, async (_req, res) => {
    res.json(getQueueStats());
});
/**
 * GET /api/v1/withdraw/:jobId
 */
withdrawalRouter.get("/:jobId", authenticate, async (req, res) => {
